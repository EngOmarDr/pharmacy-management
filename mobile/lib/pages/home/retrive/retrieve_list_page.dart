...
import 'package:pharmacy/pages/home/retrive/retrieve_retrive_page.dart';
import 'package:searchfield/searchfield.dart';

class RetrieveListPage extends StatefulWidget {
  const RetrieveListPage({super.key, required this.loginModel});

  final LoginModel loginModel;

  @override
  State<RetrieveListPage> createState() => _RetrieveListPageState();
}

class _RetrieveListPageState extends State<RetrieveListPage> {
  ...
  Future<List<Dispose>> disposeList(String pharmacyId) async {
    final response = await get(
      Uri.http(domain(), '/api/pharmacy/$pharmacyId/retrive/'),
      headers: {'Authorization': 'Bearer ${widget.loginModel.tokens.access}'},
    );
                                            ...
                                            AwesomeDialog(
                                              ...
                                              btnOkOnPress: () async {
                                                final response = await delete(
                                                  Uri.http(domain(),
                                                      '/api/pharmacy/${pharmacyId ?? widget.loginModel.pharmacyId ?? pharmacies[0].$1}/retrive/${snapshot.data![i].id}/'),
                                                  headers: {
                                                    'Authorization':
