...
import 'sales_update_page.dart';

class SalesRetrivePage extends StatefulWidget {
  const SalesRetrivePage(
      {super.key,
  ...
  final String access;
  @override
  State<SalesRetrivePage> createState() => _SalesRetrivePageState();
}

class _SalesRetrivePageState extends State<SalesRetrivePage> {
  // List<DisposeRetrive> disposeRetrive = [];

  Future<SalesRetrive?> disposeRetrive() async {
    final response = await get(
      Uri.http(
          domain(), '/api/pharmacy/${widget.pharmId}/sale/${widget.disposeId}'),
      headers: {'Authorization': 'Bearer ${widget.access}'},
    );

    print('response status : ${response.body}');

    SalesRetrive? disposesRetrive;
    if (response.statusCode == 200) {
      Map temp = json.decode(response.body);
      disposesRetrive = SalesRetrive(
          id: temp['id'],
          user: temp['user'],
          customerName: temp['coustomer_name'],
          doctorName: temp['doctor_name'],
          items: temp['items'],
          time: temp['time'],
          value: temp['value']);
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: FutureBuilder<SalesRetrive?>(
          future: disposeRetrive(),
          builder:
              (BuildContext context, AsyncSnapshot<SalesRetrive?> snapshot) {
                    ...
                    children: [
                      const Text('doctor name: ',
                          style: TextStyle(
                              fontSize: 16, fontWeight: FontWeight.bold)),
                      Text(
                        snapshot.data!.doctorName ?? 'not record',
                        textAlign: TextAlign.center,
                      ),
                    ],
                  ),
                    ...
                    children: [
                      const Text('patient: ',
                          style: TextStyle(
                              fontSize: 16, fontWeight: FontWeight.bold)),
                      Text(
                        snapshot.data!.customerName ?? 'not record',
                        textAlign: TextAlign.center,
                      ),
                    ],
                  ),
